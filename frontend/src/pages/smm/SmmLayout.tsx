import { CheckCircle2, LayoutGrid, LogOut, User } from 'lucide-react'
...
const NAV = [
  { to: '/smm/board', label: 'Board', Icon: LayoutGrid, end: true },
      ...
      <aside className="border-border bg-background/95 flex h-auto max-h-[100vh] w-full shrink-0 flex-col overflow-hidden border-b backdrop-blur md:h-[100vh] md:w-56 md:max-h-[100vh] md:border-b-0 md:border-r">
        ...
        <div className="border-border shrink-0 px-3 py-3">
          ...
          >
            ...
            >
              <User className="size-3.5" style={{ color: primary }} aria-hidden />
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-foreground truncate text-xs font-semibold">
                {user?.name ?? 'SMM'}
              </p>
              <p className="text-muted-foreground text-[10px]">Social Media</p>
            </div>
          </div>
        </div>
