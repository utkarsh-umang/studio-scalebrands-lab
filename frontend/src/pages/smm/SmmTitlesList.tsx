...
import { MOCK_SMM_TEXT_BATCH_INDEX } from '@mockData/index'
import { ClientPageHeader, formatDate } from '@/pages/client/clientPageUtils'

export function SmmTitlesList() {
  ...
  return (
    <>
      <ClientPageHeader
        title="Titles & copy"
        subtitle="Thumbnail text and video title pairs — sent to the client for approval before editing."
      />
      ...
      >
        {MOCK_SMM_TEXT_BATCH_INDEX.length === 0 ? (
          <p className="text-muted-foreground p-6 text-sm">
            No batches at the text creation step.
          </p>
        ) : (
          <ul className="divide-border divide-y">
            {MOCK_SMM_TEXT_BATCH_INDEX.map((row) => (
              <li key={row.id}>
                <Link
                  to={`/smm/titles/${row.id}`}
