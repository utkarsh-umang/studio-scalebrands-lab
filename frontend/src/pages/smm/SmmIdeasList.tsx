...
import { MOCK_SMM_IDEA_BATCH_INDEX } from '@mockData/index'
import { ClientPageHeader, formatDate } from '@/pages/client/clientPageUtils'

export function SmmIdeasList() {
  ...
  return (
    <>
      <ClientPageHeader
        title="Batch ideas"
        subtitle="Research hooks and submit idea lists for client approval (idea-first path)."
      />
      ...
      >
        {MOCK_SMM_IDEA_BATCH_INDEX.length === 0 ? (
          <p className="text-muted-foreground p-6 text-sm">
            No active idea requests.
          </p>
        ) : (
          <ul className="divide-border divide-y">
            {MOCK_SMM_IDEA_BATCH_INDEX.map((row) => (
              <li key={row.id}>
                <Link
                  to={`/smm/ideas/${row.id}`}
