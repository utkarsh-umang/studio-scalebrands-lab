...
import { MOCK_CLIENT_CLIP_BATCH_INDEX } from '@mockData/index'
import { ClientPageHeader, formatDate } from './clientPageUtils'

export function ClientBatchesList() {
  ...
  return (
    <>
      <ClientPageHeader
        title="My batches"
        subtitle="Footage you submitted — approve or reject clip ideas before editing."
      />
      ...
      >
        {MOCK_CLIENT_CLIP_BATCH_INDEX.length === 0 ? (
          <p className="text-muted-foreground p-6 text-sm">
            No clip reviews open right now.
          </p>
        ) : (
          <ul className="divide-border divide-y">
            {MOCK_CLIENT_CLIP_BATCH_INDEX.map((row) => (
              <li key={row.id}>
                <Link
                  to={`/client/batches/${row.id}`}
