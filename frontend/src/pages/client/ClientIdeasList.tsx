...
import { MOCK_CLIENT_IDEA_BATCH_INDEX } from '@mockData/index'
import { ClientPageHeader, formatDate } from './clientPageUtils'

export function ClientIdeasList() {
  ...
  return (
    <>
      <ClientPageHeader
        title="Batch ideas"
        subtitle="Idea-first batches — approve a list or ask SMM to iterate."
      />
      ...
      >
        {MOCK_CLIENT_IDEA_BATCH_INDEX.length === 0 ? (
          <p className="text-muted-foreground p-6 text-sm">
            No idea lists waiting for you.
          </p>
        ) : (
          <ul className="divide-border divide-y">
            {MOCK_CLIENT_IDEA_BATCH_INDEX.map((row) => (
              <li key={row.id}>
                <Link
                  to={`/client/ideas/${row.id}`}
                ...
                >
                  ...
                  >
                    <Zap className="size-4 text-foreground/80" aria-hidden />
                  </div>
