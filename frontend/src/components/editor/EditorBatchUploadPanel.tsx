import { CheckCircle2, FileVideo2, Image, LoaderCircle, UploadCloud } from 'lucide-react'
...
function UploadRow({ ticket, includeThumbnail }: RowProps) {
    ...
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <header className="flex flex-col gap-4 border-b border-slate-100 px-5 py-5 md:flex-row md:items-center md:justify-between">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div>
            <h2 className="text-sm font-bold text-slate-950">Upload finished production files</h2>
      ...
      {deliverables.length > 0 ? (
        <div>
          {deliverables.map((ticket) => (
            <UploadRow key={ticket.id} ticket={ticket} includeThumbnail={includeThumbnail} />
          ))}
        </div>
      ) : (
